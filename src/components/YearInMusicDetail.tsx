  ...
  return (
    <div className="fixed inset-0 z-[60] flex flex-col animate-fade-in" style={{ background: 'rgb(var(--bg-rgb))' }}>
      <div className="flex items-center gap-3 px-4 pt-4 pb-3 shrink-0">
        ...
        <h2 className="text-fg font-bold text-lg">Year in Music</h2>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-10">
        ...
        <section className="mb-8">
          ...
          {topArtists.length === 0 ? (
            <p className="text-fg/25 text-sm py-4">No plays counted yet.</p>
          ) : (
                ...
                return (
                  <div key={a.name} className="flex items-center gap-3">
                    <span className="text-[11px] font-bold w-5 h-5 rounded-full flex items-center justify-center shrink-0"
                      style={{ background: i < 3 ? `${accentColor}30` : 'rgb(var(--fg-rgb) / 0.06)', color: i < 3 ? accentColor : 'rgb(var(--fg-rgb) / 0.4)' }}>{i + 1}</span>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2 mb-1">
                        <p className="text-sm font-medium text-fg/90 truncate">{a.name}</p>
                        <span className="text-xs font-bold tabular-nums shrink-0" style={{ color: accentColor }}>{a.plays} play{a.plays === 1 ? '' : 's'}</span>
                      </div>
                      <div className="h-1 rounded-full bg-fg/8 overflow-hidden">
        ...
        <section className="mb-8">
          ...
          {peakLabel === null ? (
            <p className="text-fg/25 text-sm py-4">No listening sessions recorded yet.</p>
          ) : (
            <>
              ...
              <div className="flex gap-[3px] mt-1 text-[8px] text-fg/25">
                {HOUR_LABELS.map((l, h) => (
        ...
        <section className="mb-8">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-2xl p-4" style={{ background: 'rgb(var(--fg-rgb) / 0.04)', border: '1px solid rgb(var(--fg-rgb) / 0.06)' }}>
              ...
              <p className="text-fg/40 text-xs mt-1">Longest streak (days)</p>
            </div>
            <div className="rounded-2xl p-4" style={{ background: 'rgb(var(--fg-rgb) / 0.04)', border: '1px solid rgb(var(--fg-rgb) / 0.06)' }}>
              <p className="text-3xl font-extrabold tabular-nums text-fg">{streak.current}</p>
              <p className="text-fg/40 text-xs mt-1">Current streak (days)</p>
            </div>
          </div>
        </section>
        ...
        <section>
          <h3 className="text-fg/60 text-xs font-semibold uppercase tracking-wider mb-3">Genre Breakdown Over Time</h3>
          {fullHistory === null ? (
            <p className="text-fg/25 text-sm py-4">Loading…</p>
          ) : genreOverTime.buckets.length === 0 ? (
            <p className="text-fg/25 text-sm py-4">Tag some songs with a genre to see this over time.</p>
          ) : (
            <>
              <div className="flex flex-wrap gap-3 mb-4">
                {genreOverTime.topGenres.map((g, i) => (
                  <span key={g} className="flex items-center gap-1.5 text-xs text-fg/50">
                ...
                {genreOverTime.buckets.map((bucket) => {
                  ...
                  return (
                    <div key={bucket.key} className="flex items-center gap-3">
                      <span className="text-xs text-fg/40 w-16 shrink-0">{bucket.label}</span>
                      <div className="flex-1 h-4 rounded-full overflow-hidden flex bg-fg/5">
                        {genreOverTime.topGenres.map((g, i) => {
                      ...
                      <span className="text-xs text-fg/30 tabular-nums w-8 text-right shrink-0">{total}</span>
                    </div>
                  );
                })}
              </div>
