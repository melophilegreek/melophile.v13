  ...
  return (
    <div className="relative h-full overflow-hidden rounded-2xl shadow-panel"
      ...
      {/* Feature (Minimized Now Playing bar): compact single-row bar for
          ...
          play/pause button, and a plain (non-interactive) progress line --
          everything else (artist name, shuffle/skip/queue/lyrics/etc.) is
          intentionally left out. The progress line sits flush along the
          bottom edge of the card rather than reusing <SeekBar>, since this
          mode is meant to be a glance-only summary, not another set of
          controls. Pairs with the shorter h-[68px] mobile container height
          set in App.tsx for this mode. */}
      {playerBarStyle === 'minimized' && (
        <div className="md:hidden relative h-full flex items-center gap-3 px-4">
          ...
          {/* Plain progress line, flush to the bottom edge of the card --
              read-only (no seek/drag), matching "a simple progress bar". */}
          <div className="absolute left-0 right-0 bottom-0 h-1 bg-fg/10 overflow-hidden rounded-b-2xl">
