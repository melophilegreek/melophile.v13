...
export function EditTagsModal({ song, accentColor, onClose, onUpdated }: Props) {
  ...
  const fieldClass = 'w-full px-3 py-2 rounded-lg bg-fg/5 border border-fg/10 text-fg text-sm placeholder:text-fg/25 focus:outline-none focus:border-fg/30 transition-colors';
  const labelClass = 'block text-fg/40 text-[11px] font-semibold uppercase tracking-wider mb-1.5';
    ...
    <div ref={overlayRef} className="fixed inset-0 z-50 flex items-center justify-center px-4"
      ...
      <div className="w-full max-w-sm rounded-2xl p-5 shadow-2xl animate-slide-up"
        ...
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-fg font-bold">Edit Tags</h3>
        ...
        <button onClick={handleReset} disabled={resetting}
          className="flex items-center gap-1.5 text-fg/40 hover:text-fg/70 text-xs mt-4 transition-colors disabled:opacity-50">
        ...
        <div className="flex gap-2 mt-3">
          ...
          <button onClick={handleSave} disabled={!canSave}
            className="flex-1 py-2.5 rounded-xl text-fg font-semibold text-sm transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
